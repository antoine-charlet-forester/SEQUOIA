...
# Sélection des communes concernées
  ...
  COMMUNE <- st_intersection(COMS_SF, TEMPON1)
  cat(" \n", nrow(COMMUNE), 'communes ont été détectées'," \n")
...
# Fonction de téléchargement
  ETALABCA <- function(ID_CAD, N) { # Debut fonction ETALABCA
    ...
    if(nrow(SHPS_SF)>0){
      ...
      return(SHPS_SF)
      cat('La donnée', DATA_LIST[N], 'a été téléchargée sur l emprise', '\n')
    }
...
# Import des données EDIGEO
  ...
  cat(" \n", paste(nrow(FEUILLE), "feuilles cadastrales ont été détectées")," \n")
  ...
  for (b in 1:nrow(FEUILLE)) {
    ...
    download.file(ZIP_URL, TF, method="libcurl") # Téléchargement du fichier URL sous le nom TF dans TD

    setwd(TD)
    bunzip2(TF)
    untar(str_replace(TF,".bz2",""))
    cat(" \n", paste("La feuille ", FEUILLE[b,1], "a été téléchargée")," \n")
  }
