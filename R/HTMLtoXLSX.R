...
HTMLtoXLSX <- function(rephtml=F,repRdata=NULL) {

# Import des données .html
  if(isFALSE(rephtml)) { # Sélection du répertoire
    rephtml <- dlg_dir(default= getwd(), title = "Choisir le repertoire des matrices .HTML")$res
  }

  if (!is.null(repRdata)){ # Import ou téléchargement des données administratives
    load(repRdata)
  } else {
    SEQUOIA:::INSEEtoRDATA()
  }

  if(rlang::is_empty(rephtml)) { # Si le répertoire est vide : Absence de traitement
  message("Pas de dossier sélectionnés > Traitement annulé","\n" )
...
## Récupération des données cadastrales
  ...
  ### Sélection des colonnes nommées ci-dessous
    ...
    #### Détecte si la liste contient des données foncières
          if (!is.null(data[[j]][1,3])){
            if(!is.na(data[[j]][1,3])){
              if(data[[j]][1,3]=="LIVRE FONCIER"){
  ...
  ### Inscription des données générales
        df <- df %>% filter(NATURE !="Â") %>% # Conservation des seules subdivisions fiscales
          ...
          dplyr::select(DEP_CODE, COM_CODE,COM_NOM,PROP,ADRESSE,SECTION:NATURE,REV_CAD,SURFACE) # Mise en ordre des champs
...
## Sortie définitive d'un .xlsx
      ...
      cat(paste("Liste parcelles cadastrales enregistree dans le rephtmlertoire : ", repOut),"\n")
